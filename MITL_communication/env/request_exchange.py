    ...
    while world.agents_light.num_round < num_round_exchange:
        ...
        while i < num_agent_heavy:
            ...
            for j in range(len(path_sweep[i])):
                ...
                if (math.sqrt(abs(path_sweep[i][j][0] - world.agents_heavy[i].p_pos[0]) ** 2 +
                              abs(path_sweep[i][j][1] - world.agents_heavy[i].p_pos[1]) ** 2) <= r_0 and
            ...
            for j in range(len(path_ready_to_sweep)):
                ...
                # meet heavy-duty agents
                if (math.sqrt(abs(path_ready_to_sweep[j][0] - world.agents_heavy[ii].p_pos[0]) ** 2 +
                              abs(path_ready_to_sweep[j][1] - world.agents_heavy[ii].p_pos[1]) ** 2 ) <= r_0 and
                    ...
                    print("i: ", i, " ii: ", ii, "j: ", j, 'len(path_sweep)', len(path_sweep), 'len(path_sweep[ii])',
                          len(path_sweep[ii]))
